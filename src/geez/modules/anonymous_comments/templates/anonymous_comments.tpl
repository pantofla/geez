...
<fieldset><legend>{#PLIGG_Visual_Comment_Send#}</legend>	
	<form action="" method="POST" id="thisform" onsubmit="javascript:return validate('thisform','a_email');">
		<span><b>&nbsp;User Name :&nbsp;</b></span><br/><input  type ="text" name = "a_username" />{$a_username}<br clear="all" />
{*		<span><b>&nbsp;Website :&nbsp;</b></span><br/><input  type ="text" name = "a_website" />{$a_website}<br clear="all" />
		<span><b>&nbsp;E-Mail :&nbsp;</b></span><br/><input  type ="text" name = "a_email" />{$a_email}<br clear="all" />
*}
		<label>{#PLIGG_Visual_Comment_NoHTML#}</label><br clear="all" />
...
{config_load file="/languages/lang_english.conf"}