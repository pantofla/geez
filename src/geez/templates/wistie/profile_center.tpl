...
{checkActionsTpl location="tpl_pligg_profile_info_start"}
		...
		<h2>{#PLIGG_Visual_Profile_UploadAvatar2#}:</h2>
			{if $user_avatar_source eq "useruploaded"}
				...
				<form method="POST" enctype="multipart/form-data" name="image_upload_form" action="{$form_action}">
					...
					<input type="submit" value="{#PLIGG_Visual_Profile_AvatarUpload#}" name="action" class="log2 searchbutton"><br>
				</form> 
			{/if}	
	{/if}
	...
	{if $UseAvatars neq false}
			...
			<input type=submit name="save_profile" value="{#PLIGG_Visual_Profile_Save#}" class="log2 searchbutton">					
	{/if}
...
{checkActionsTpl location="tpl_pligg_profile_end"}
<input type="submit" name="save_profile" value="{#PLIGG_Visual_Profile_Save#}" class="log2 searchbutton" tabindex="16"></p>
