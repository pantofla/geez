...
<fieldset><legend>{#PLIGG_Visual_Recommend_FormTitle#}</legend>
	<form name="email_to" method="get" action="{$my_base_url}{$my_pligg_base}/recommend.php">
		...
		<textarea name="email_message{$link_shakebox_index}" id="email_message{$link_shakebox_index}" cols="40" rows="5"></textarea><br />
{*
...
*}
		<br /><input type="button" name="email_to_submit" value="{#PLIGG_Visual_Recommend_Submit#}" onclick="emailto ({$link_id}, {$link_shakebox_index}, '{$instpath}', 3);" class="log2 searchbutton">
<a href="javascript://" onclick="show_recommend({$link_shakebox_index}, {$link_id}, '{$instpath}');"><strong> close </strong> </a>
	</form>
</fieldset>
</div>